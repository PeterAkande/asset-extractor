...
def get_cached_result_by_id(result_id: str) -> Optional[Dict[str, Any]]:
    ...
    if result_json:
        try:
            ...
            data["result_id"] = result_id  # Add the result ID to the data
            return data
...
def store_result_in_cache(
    url: str, result: Dict[str, Any], result_id: Optional[str] = None
) -> str:
    """
    ...
    redis_client.set(
        f"url:{url}", result_id, ex=CACHE_EXPIRATION  # Set same expiration time
    )
